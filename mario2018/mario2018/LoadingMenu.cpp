...
void LoadingMenu::Draw(SDL_Renderer* rR) {
	if (loadingType) {
		...
		CCFG::getText()->Draw(rR, "HANOI: ", 320, 144);
		...
		CCFG::getText()->DrawCenterX(rR, "PETERHAO VO DICH, CHAY THI GIU " + CCFG::getKeyString(CCFG::keyIDShift), 400, 16);
	}
	else {
		CCore::getMap()->DrawGameLayout(rR);
		CCFG::getText()->DrawCenterX(rR, "PETER CHET CMNR", 240, 16);
	}
}
