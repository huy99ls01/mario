...
void Mushroom::Draw(SDL_Renderer* rR, CIMG* iIMG) {
	if (minionState >= 0) {
		...
		if (inSpawnState) {
			CCore::getMap()->getBlock(CCore::getMap()->getLevelType() == 0 ? 4 : 42)->getSprite()->getTexture()->Draw(rR, (int)fXPos + (int)CCore::getMap()->getXPos(), (int)fYPos + (32 - inSpawnY) - CCore::getMap()->getMapBlock(iX, iY)->getYPos() + 2, false);
		}
	}
}
